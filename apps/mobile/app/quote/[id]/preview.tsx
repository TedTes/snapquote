...
export default function QuotePreviewScreen() {
  ...
  const sendQuote = useMvpStore((state) => state.sendQuote);
    ...
    <Screen>
      ...
      <SheetModal onDismiss={() => setShowSendSheet(false)} style={styles.modalBackdrop} visible={showSendSheet}>
        <AnimatedSheetContent style={styles.sheet}>
          ...
          <PrimaryButton disabled={!customer?.email} label="Send email now" onPress={confirmSend} />
          <Pressable accessibilityRole="button" onPress={() => setShowSendSheet(false)} style={styles.cancelLink}>
