...
describe("price book state", () => {
  ...
  it("creates and updates a local custom price item", () => {
    useMvpStore.getState().addPriceBookItem({
  ...
  it("confirms a yellow line and increments the matched item usage count", () => {
    ...
    useMvpStore.getState().confirmYellowLine("quote-local", "line-local");

    const confirmed = useMvpStore.getState().priceBookItems[0];
    const line = useMvpStore.getState().quotes[0]?.lineItems[0];
  ...
  it("archives a local price item from the active list", () => {
    ...
    useMvpStore.setState({ priceBookItems: [active] });

    useMvpStore.getState().archivePriceBookItem(active.id);

    expect(useMvpStore.getState().priceBookItems).toEqual([]);
  });
  ...
  it("keeps remote price items over local items with the same key during hydration", () => {
    ...
    useMvpStore.setState({ priceBookItems: [localConflict, localUnique] });
    ...
    expect(
      useMvpStore.getState().priceBookItems.map((candidate) => candidate.name),
