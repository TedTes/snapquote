...
export function DemoPublicQuoteScreen({ playback }: DemoPublicQuoteScreenProps) {
  ...
  const scrollProgress = Math.min(Math.max((playback.elapsedMs - 760) / 900, 0), 1);
  const scrollOffset = Math.round(scrollProgress * 188);

  return (
    <BrowserFrame address="quotevan.com/q/8f21ac">
      <div className="qv-flow-public-scroll" style={{ transform: `translateY(-${scrollOffset}px)` }}>
        <article className="qv-flow-public-quote">
          ...
          <p className="qv-flow-public-terms">
        ...
        <div className="qv-flow-scroll-hint" aria-hidden="true">
          <span style={{ transform: `translateY(${scrollProgress * 18}px)` }} />
        </div>
      </div>
    </BrowserFrame>
  );
